...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/audio-trim/sw.js, scope=/audio-trim/）。
    // scope がツールの全 locale ページ（/audio-trim/, /audio-trim/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/audio-trim/sw.js', {
      scope: '/audio-trim/',
    });
